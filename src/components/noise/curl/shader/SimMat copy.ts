...
export default class SimMatCurly extends ShaderMaterial {
  constructor(pos: DataTexture, offset: DataTexture) {
    ...
    void main(){
      ...
      vel *= nc.xy *0.2 ;
      pos.x += vel.x * length(nc.xy) * cos(an * nc.z) * 0.075   ;
      pos.y += vel.y * length(nc.xy) * sin(an * nc.z) * 0.085  ;
      // float timer = mod(uTime*0.1,1.);
      // float sc = map(sin(timer *2.*PI),-1.,1.,2.0,8.);
      // vec3 nc = curl(vec3(pos.x*floor(sc),pos.y*floor(sc), uTime*0.1))*5.   ;
      // float an = atan(nc.x, nc.y);
      // vel *= nc.xy *0.05 ;
      // pos.x += vel.x * cos(an ) * 0.075   ;
      // pos.y += vel.y * sin(an ) * 0.085  ;
